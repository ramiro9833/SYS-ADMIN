# lib/powershell/SSL.ps1 - Generación de certificados y configuración SSL/TLS en Windows
...
function Generar-Certificado-SelfSigned-Win {
    ...
    if ($null -ne $cert) {
        ...
        Write-Host "[ERROR] Falló la generación del certificado SSL." -ForegroundColor Red
        return $null
    }
}
...
function Configurar-SSL-IIS-Win {
    ...
    # 3. Habilitar redirección HTTP -> HTTPS usando URL Rewrite si está disponible
    # Asegurar módulo URL Rewrite
    if (-not (Test-Path "$env:SystemRoot\System32\inetsrv\rewrite.dll")) {
        Write-Host "[INFO] Módulo URL Rewrite no detectado. Instalando..." -ForegroundColor Yellow
        choco install urlrewrite -y --no-progress | Out-Null
    }

    # Crear/Sobrescribir el web.config en wwwroot para HSTS y Redirección
...
"@
    ...
    iisreset
    Write-Host "[OK] IIS configurado con SSL y Redirección HTTPS activa." -ForegroundColor Green
}
...
function Configurar-SSL-Apache-Win {
    ...
    if (-not $apacheBase) {
        Write-Host "[ERROR] No se pudo encontrar la ruta de instalación de Apache." -ForegroundColor Red
        return
    }
    ...
    if (-not $ok) {
        Write-Host "[ERROR] Falló la exportación del certificado PEM para Apache." -ForegroundColor Red
        return
    }
...
"@
    ...
    # Forzar redirección HTTP -> HTTPS en el puerto 8080 (o el puerto configurado de apache)
    ...
    # Modificar el puerto de escucha para redirección
    $redirectVhost = @"
<VirtualHost *:$puerto>
...
"@
    # Asegurar que rewrite_module esté activo
    ...
    Write-Host "[OK] Apache Windows configurado con SSL (443) y Redirección HTTPS." -ForegroundColor Green
}
...
function Configurar-SSL-Nginx-Win {
    ...
    if (-not $nginxBase) {
        Write-Host "[ERROR] No se pudo encontrar la ruta de instalación de Nginx." -ForegroundColor Red
        return
    }
    ...
    if (-not $ok) {
        Write-Host "[ERROR] Falló la exportación del certificado PEM para Nginx." -ForegroundColor Red
        return
    }
    ...
    $puertoHttp = 80
    # Escribir un nginx.conf limpio con SSL y Redirección
    $nginxContent = @"
worker_processes  1;
...
"@
    ...
    Write-Host "[OK] Nginx Windows configurado con SSL y Redirección HTTPS." -ForegroundColor Green
}
...
function Configurar-SSL-IISFTP-Win {
    ...
    # Establecer la configuración SSL en el servidor FTP a nivel global/sitio
    try {
